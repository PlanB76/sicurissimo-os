...
### 4.2 NETWORK81+ → NETWORK81+
**Stato: CONFORME — nessuna correzione necessaria ✓**

Le occorrenze di "NETWORK81+" sono SOLO in:
- `.htaccess` righe 28-30, 86-88: redirect legacy `siconet.online` → `81plus.network` (corretto)
- `data/kb/listino.txt`: "NETWORK81+ ex NETWORK81+" (contesto storico ammesso)
- `sito81.js` riga 122: regex routing legacy (non copy pubblico)

Nessuna occorrenza nel copy pubblico che citi NETWORK81+ come nome attivo.

### 4.3 81plus.digital → 81plus.digital
**Stato: CONFORME — nessuna correzione necessaria ✓**
...
### 5.5 DEX
**Stato: CONFORME ✓**
Appare solo come negazione: "Solo DEX, nessun DEX", "niente DEX per ora".
...
| 3 | `greengrove.html` | `Green81+` → `Green81+` in titolo, h1, copy | ✓ |
| 4 | `zone_modello.html` | `Green81+` → `Green81+` | ✓ |
| 5 | `chi-siamo.html` | `Green81+` → `Green81+` | ✓ |
| 6 | `christmas.html` | `Green81+` → `Green81+` | ✓ |
| 7 | `flowchart3d.html` | `GREENGROVE81` → `GREEN81+` | ✓ |
| 8 | `LOGICA_FUNZIONALE_81PLUS.md` | `Green81+` → `Green81+` | ✓ |
| 9 | `FOTOGRAFIA_81PLUS_13_GIUGNO_2026.md` | `Green81+` → `Green81+` | ✓ |
